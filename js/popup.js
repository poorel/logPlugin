s$(function() {
	...
	// 初始化国际化
	s$('#test_i18n').html(chrome.i18n.getMessage("helloWorld"));


});

// 打开后台页
s$('#open_background').click(e => {
	window.open(chrome.extension.getURL('background.html'));
});

// 调用后台JS
s$('#invoke_background_js').click(e => {
	var bg = chrome.extension.getBackgroundPage();
	bg.testBackground();
});

// 获取后台页标题
s$('#get_background_title').click(e => {
	var bg = chrome.extension.getBackgroundPage();
	alert(bg.document.title);
});

// 设置后台页标题
s$('#set_background_title').click(e => {
...
// 打开新窗口
s$('#open_new_window').click(() => {
	chrome.windows.create({state: 'maximized'});
});

// 关闭全部
s$('#close_current_window').click(() => {
	chrome.windows.getCurrent({}, (currentWindow) => {
...
// 新标签打开网页
s$('#open_url_new_tab').click(() => {
	chrome.tabs.create({url: 'https://www.baidu.com'});
});

// 当前标签打开网页
s$('#open_url_current_tab').click(() => {
...
// 高亮tab
s$('#highlight_tab').click(() => {
	chrome.tabs.highlight({tabs: 0});
});

// popup主动发消息给content-script
s$('#send_message_to_content_script').click(() => {
...
s$('#connect_to_content_script').click(() => {
	getCurrentTabId((tabId) => {
